...
import type { HardwareWallet, HardwareWalletAccount, Wallet, WalletSelector } from '@near-wallet-selector/core';
...
export const DerivationPath: React.FC<DerivationPathProps> = ({
  ...
  const [route, setRoute] = useState<HardwareRoutes>(!!initError ? 'ErrorRoute' : 'EnterDerivationPath');
  const [derivationPath, setDerivationPath] = useState(DEFAULT_DERIVATION_PATH);

  const [error, setError] = useState<string | JSX.Element>(initError as any);
  ...
  const getAccountIds = async (publicKey: string): Promise<Array<string>> => {
    ...
    if (typeof getConfig().kitWalletOn === 'boolean' && !getConfig().kitWalletOn) {
      throw new Error();
    } else {
      return accountIds;
    }
  ...
  const resolveAccounts = async (wallet: Wallet): Promise<Array<HardwareWalletAccountState> | null> => {
    const publicKey = await (wallet as HardwareWallet).getPublicKey(derivationPath);
    try {
      ...
      if (noAccounts) {
        setError(
          <span>
            ...
            <a
              href={`https://${
                selector.options.network.networkId === 'testnet' ? 'testnet' : 'app'
  ...
  const handleAddCustomAccountId = async () => {
    try {
      ...
      const publicKey = await (hardwareWallet as HardwareWallet).getPublicKey(derivationPath);
      setAccounts([
        {
  ...
  const handleEnterClick: KeyboardEventHandler<HTMLInputElement> = async (e) => {
    if (e.key === 'Enter') {
            ...
            <div className="bg-black bg-opacity-20 rounded-md flex items-center justify-center mx-10">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setDerivationPath(BASE_DERIVATION_PATH + e.target.value + "'");
                }}
                style={{
                  minWidth: '14px',
      ...
      {route === 'OverviewAccounts' && (
        <div className="w-full">
          {accounts.map((account, index) => (
            <div key={account.accountId} className="pt-10 text-center text-base font-bold">
