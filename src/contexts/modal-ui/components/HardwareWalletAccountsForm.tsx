...
const HardwareWalletAccountsForm: React.FC<FormProps> = ({ accounts, onSelectedChanged, onSubmit }) => {
  return (
        ...
        <div>
          <div className="px-10 flex flex-col">
            {accounts.map((account, index) => (
              <div key={index} className="w-full mt-3 relative px-4 py-3 bg-black text-sm bg-opacity-10 rounded-md">
                <label className=" " htmlFor={account.accountId}>
                  {' '}
                  {account.accountId}
                </label>

                <LedgerCheckbox checked={account.selected} index={index} setChecked={onSelectedChanged} />
