  ...
  return (
    <div className="flex items-center pt-5 justify-center text-xs">
      <span className="text-xs">
        <FormattedMessage id="first_time_using_ref" defaultMessage="First time using Ref" />?
      </span>
      <div
    ...
    <Fragment>
      <div className="wallet-options-wrapper">
        <ul className={'options-list'}>
          {modules.reduce<Array<JSX.Element>>((result, module, currentIndex) => {
            const { selectedWalletId } = selector.store.getState();
            const { name, description, iconUrl, deprecated } = module.metadata;
            const selected = module.id === selectedWalletId;
            if (isMobile && module.type !== 'browser' && module.id !== 'meteor-wallet') {
              return result;
            }

            const installed = module.type === 'injected' && module.metadata.available && module.id !== 'meteor-wallet';
            ...
            result.push(
              <li
                ...
                className={`px-5 py-3 relative ${hoverOption === currentIndex ? 'bottom-1' : ''} ${
              ...
              >
                <div className="absolute top-0 right-0">
                  {selected ? <SelectedIcon /> : installed ? <Installed /> : isBeta ? <Beta /> : null}
                </div>

                <div title={description || ''} className="wallet-content flex items-center">
                  ...
                  <div className="flex items-start flex-col text-sm">
                    <span className="">{name === 'NEAR Wallet' ? 'NEAR' : name}</span>

                    <span className="text-xs text-primaryText official-url">{walletOfficialUrl[name]}</span>
                  </div>
                </div>
              </li>
            );
      ...
      {isMobile && (
        <div className="flex flex-col items-center mt-7">
          <div className="text-xs mb-4">
            <FormattedMessage id="wallets_below_supports_on_PC" defaultMessage={'Wallets below support on PC'} />
          </div>

          <div className="flex items-center opacity-50">
            {notSupportingIcons.map((url, i) => {
              return <img key={i} src={url} className="w-7 h-7 mr-4" alt="" />;
            })}
          </div>
        </div>
      )}
