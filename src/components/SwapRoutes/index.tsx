  ...
  if (tokens.length == 3) {
    return (
      <div className="text-white flex items-center justify-between">
        ...
        <Hub token={tokens[1]} poolId={pools?.[0]?.id} Dex={pools?.[0]?.Dex as any} />
        <div className="px-3">
          <ArrowRight />
        </div>

        <Hub token={tokens[2]} poolId={pools?.[1]?.id} Dex={pools?.[0]?.Dex as any} />
      </div>
    );
  ...
  return (
    <div className="flex items-center text-xs text-white">
      ...
      {route.length === 1 ? (
        ...
        >
          <Icon token={(route[0] as any).tokens[0]} size={'5'} />
          <div
            style={{
              fontSize: '10px',
              opacity: '0.5',
          ...
          <CrossIcon
            Icon={<Icon token={(route[0] as any).tokens[1]} size={'5'} />}
        ...
        <div className="flex w-full items-center justify-between relative">
          ...
          >
            ...
            <PoolName dex={route[0].pool.Dex as any} translate="0" />
          </div>
          ...
          >
            <CrossIcon
              Icon={<Icon token={(route[0] as any).tokens[1] as any} size="5" />}
              poolId={route[0].pool.id}
            />
          </div>
          ...
          >
            <PoolName dex={route[1].pool.Dex as any} translate="15" />
          </div>

          <div className="absolute right-0">
            <CrossIcon
              Icon={<Icon token={(route[0] as any).tokens[2]} size="5" />}
