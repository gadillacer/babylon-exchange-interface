...
export const failToastAccount = (errorMsg?: string) => {
  toast(
    <a
      ...
      }/address/${(getCurrentWallet() as any).wallet?.getAccountId()}`}
      target="_blank"
      rel="noreferrer"
      style={{
        lineHeight: '20px',
...
export const checkCrossSwapTransactions = async (txHashes: string[]) => {
  ...
  const txDetail: any = await checkTransaction(lastTx as any);
  ...
  if (txHashes.length > 0) {
    ...
    if (isAurora && ifCall) {
      ...
      if (
        erc20FailPattern.test(parsedOut as any) ||
        ...
        const secondDetail = await checkTransaction(secondLastHash as any);
        ...
        if (slippageErrprReg.test(parsedOutput as any)) {
          return {
            hash: secondLastHash,
            status: false,
            errorType: 'Slippage Violation',
          };
        } else if (expiredErrorReg.test(parsedOutput as any)) {
          return {
            hash: secondLastHash,
            status: false,
            errorType: 'Expired',
