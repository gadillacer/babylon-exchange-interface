...
export default function InputAmount({
  ...
  const handleChange = (amount: string) => {
    ...
    (ref as any).current.value = amount;
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          {forSwap ? <span className="mr-3">{tokenPrice(price as any)}</span> : null}
        </div>
      </fieldset>
    </>
  );
...
export function NewFarmInputAmount({
  ...
  const handleChange = (amount: string) => {
    ...
    (ref.current as any).value = amount;
  };
      ...
      >
        ...
        <div className="flex items-center ">
          <input
            ...
            max={max}
            min="0"
            onWheel={() => (ref as any).current.blur()}
            // {...rest}
            step="any"
          ...
          <span className="flex items-center">
            <a
              className={`rounded-lg flex items-center justify-center text-center  ${
                rest.value === max && !ONLY_ZEROS.test(max as any)
              ...
              onClick={() => handleChange(max as any)}
            >
              <span>Max</span>
            </a>
