...
export const StableSelectToken = ({
  ...
  const handleSelect = (token: TokenMetadata) => {
    ...
    if (onTokenBTC && !BTCtokens.find((t) => t?.id === postSelected.id)) {
      onSelectPost(BTCtokens.find((t) => t?.id !== token.id) as any);
    } else if (onTokenUSD && !USDtokens.find((t) => t?.id === postSelected.id)) {
      onSelectPost(USDtokens.find((t) => t?.id !== token.id) as any);
    } else if (
      onTokenNEAR &&
      !NEARtokens.find((t) => t?.id === postSelected.id)
    ) {
      onSelectPost(NEARtokens.find((t) => t?.id !== token.id) as any);
    }
  };
            ...
            return (
              <div
                ...
                onClick={(e) => {
                  ...
                  setVisible(!visible);
                  handleSelect(token as any);
                }}
              >
                <span className="text-white font-semibold text-sm">
                  {toRealSymbol((token as any).symbol)}
                </span>
                <span>
                  {token?.icon ? (
                    <img
        ...
        <section className="text-white">
          ...
          {showCommonBasses && !forCross && (
            <CommonBasses
              ...
              tokenPriceList={tokenPriceList as any}
            />
          )}
          <Table
            sortBy={sortBy}
